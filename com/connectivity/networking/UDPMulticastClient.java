 ...
 *
 */
public class UDPMulticastClient extends BaseSelectableUDPChannel {
	 ...
	 * @param protocolName
	 */
	public UDPMulticastClient(InetSocketAddress multicastGroup, String protocolName) {
		// Force IPV4 usage
		// Enables re-use address
		// Bind to ANY_ADDR + Protocol port
		// Enforcing all network interfaces (valid ones) register
		try {
			transport = DatagramChannel.open(StandardProtocolFamily.INET).setOption(StandardSocketOptions.SO_REUSEADDR,
					true);
